...
import { computed, onMounted, ref, onBeforeUnmount, shallowRef, nextTick } from "vue";
...
const rules = {
  ...
  typeId: [{ required: true, message: '请选择物种', trigger: 'change' }],
  intro: [{ required: false, message: '请输入简介', trigger: 'blur' }],
  description: [{ required: false, message: '请输入描述', trigger: 'blur' }],
  season: [{ required: false, message: '请输入最佳动物季节', trigger: 'blur' }],
...
const handleClose = (tag) => {
  ...
  form.value.tags = JSON.stringify(tagsList.value);
};
...
const handleInputConfirm = () => {
  if (inputValue.value) {
    tagsList.value.push(inputValue.value);
    form.value.tags = JSON.stringify(tagsList.value);
  }
...
const getTableData = async () => {
  const res = await http.get(
    `/animal/page?pageNum=${params.value.pageNum}&pageSize=${params.value.pageSize}&name=${params.value.name}`
  );
  if (res.code === 200) {
...
const handleEdit = (row) => {
  form.value = { ...row };
  tagsList.value = formatTags(row.tags);
...
const delRow = async (id) => {
  const res = await http.get(`/animal/del?id=${id}`);
  if (res.code === 200) {
...
const batchDel = async () => {
  ...
  const res = await http.post("/animal/batchDel", selectedIds.value);
  if (res.code === 200) {
...
const submit = async () => {
  if (isEdit.value) {
    const res = await http.post(`/animal/edit?id=${form.value.id}`, {
      ...form.value,
      tagList: tagsList.value
    });
    if (res.code === 200) {
    ...
    const res = await http.post(`/animal/add`, {
      ...form.value,
      tagList: tagsList.value
    });
    if (res.code === 200) {
  ...
  <div>
    <header class="flex">
      <div class="flex form-item">
        <p class="label-text">动物名称：</p>
        <el-input v-model="params.name" placeholder="请输入动物名称" />
      </div>
    ...
    <main>
      ...
      >
        ...
        <el-table-column prop="typeName" label="物种名称" width="120" />
        ...
        <el-table-column prop="season" label="最佳动物季节" width="120" />
    ...
    <el-dialog v-model="visible" :title="dialogTitle" width="800" top="2%" style="max-height: 800px; overflow: hidden; overflow-y: scroll;">
      <el-form :model="form" label-width="120">
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="物种ID" prop="typeId">
              <el-select v-model="form.typeId" placeholder="请选择物种" style="width: 100%">
                <el-option
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="最佳动物季节" prop="season">
              <el-input v-model="form.season" placeholder="请输入最佳动物季节" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="图片上传" prop="coverUrl">
              <el-upload
