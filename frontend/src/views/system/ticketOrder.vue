...
const getTableData = async () => {
  const res = await http.get(
    `/ticketOrder/page?pageNum=${params.value.pageNum}&pageSize=${params.value.pageSize}&orderNo=${params.value.orderNo}&status=${params.value.status}&userId=${params.value.userId}`
  );
  if (res.code === 200) {
  ...
  <div>
    <header class="flex">
      ...
      <div class="flex form-item">
        ...
        <el-input v-model="params.userId" placeholder="请输入用户ID" clearable style="width: 200px;" />
      </div>
    ...
    <main>
      <div class="flex op-box">
        <el-button
          v-if="hasPermission('ticketOrder', '删除')"
          :icon="Delete"
          type="danger"
