...
const getTableData = async () => {
  const res = await http.get(
    `/forumPost/page?pageNum=${params.value.pageNum}&pageSize=${params.value.pageSize}&title=${params.value.title}&userId=${userInfo.id}`
  );
  if (res.code === 200) {
  ...
  <div style="width: 80%; margin: 0 auto;">
    ...
    <main>
      ...
      >
        ...
        <el-table-column prop="animalName" label="关联动物" width="120" />
        <el-table-column prop="locationName" label="动物地点" />
        <el-table-column prop="observationTime" label="动物时间" />
